...
The firmware on the EMILY's pixhawk mini was out of date, no longer supported by the most recent [QGroundControl](http://qgroundcontrol.com/). Choices were either to use an old QGroundControl or update the firmware. I opted for updating the firmware. First I tried the latex PX4 with a rover frame. I don't remember exactly what was going wrong, but the behavior was buggy. Note that I tried PX4 before fixing the loose connection, so perhaps PX4 was fine. I tried ArduRover in Boat mode, which was working as expected right away. The problem is ArduRover's lack of joystick support as well as the myriad advanced [PX4 features](https://docs.px4.io/master/en/advanced_features/) that I am interested in. We have two Pixhawk minis, so I intend to maintain one as ArduRover since it is working and see if I can get PX4 on the other. 

The final issue I had is a bit of an embarassment. Steering was working perfectly but only a slight noise and no action occured with the throttle. After a significant amount of time checking connections, Pixhawk and RC settings.. I finally found that the motor for throttle has two connections and one was unplugged. Since, as I recall, the boat has been unused since the initial demo from CRASAR I never would have expected anything to be disconnected and assumed anything wrong was from my own settings. 
