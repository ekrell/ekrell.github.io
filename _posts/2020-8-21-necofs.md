...
I have previously discussed that [Boston Harbor](https://www.nps.gov/boha/planyourvisit/maps.htm) presents a very nice path planning environment since it has both numerous obstacle islands and available water current forecasts.
...
the can produce a plot instead of all the bands which is much more efficient for exploring. Below, each row of the chart is one hour's forecast. 
